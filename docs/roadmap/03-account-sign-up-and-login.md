...
Built as eight task slices; the breakdown under `docs/tasks/03-…/` was retired
on completion. The decisions and deviations that outlive it:

- **The auth Drizzle schema is generated by a script of ours, not
  `@better-auth/cli`.** `auth.md`'s Commands table names `better-auth generate`,
  but that package (latest `1.5.0-beta`) omits `account.issuer` and the
  `(issuer, accountId)` unique index that `better-auth` 1.7.2 validates at
  adapter init — a CLI schema makes every `auth.api` call throw.
  `apps/api/scripts/generate-auth-schema.ts` emits the file from
  `better-auth/db`'s own `getAuthTables`, so it cannot drift from the library;
  `make api-auth-schema` runs it and `make check` drift-checks the output.

- **The `auth` module owns a small Postgres pool of its own.**
  `architecture-api.md` rule 19 and `auth.md` rule 1 forbid it reaching into the
  `system` module's `DATABASE` provider, so it builds its own `postgres()` from
  the validated env. One database (`stack-api.md` rule 26), two pools.

- **`asResponse: true` handed back the `set-cookie`** (the Blockers' open
  question). Every use case calls `auth.api.*({ …, asResponse: true })`; Better
  Auth returns a `Response` for success and failure alike, normalised to
  `{ ok, status, body, setCookie }`. `returnHeaders` was not needed.

- **The rate limit is a guard of ours, not `@nestjs/throttler`.** 6.5.0 is
  CJS-only, peers `@nestjs/common <= 11`, and its `require()` of NestJS 12's ESM
  `@nestjs/common` breaks every Jest integration suite under `--experimental-vm-
  modules`. `SignInRateLimitGuard` is one in-process sliding window, two keys
  (source address, submitted e-mail), no dependency. `stack-api.md` rule 39 and
  `auth.md` rule 17 name the package and should be revisited.

- **`system` may depend on `auth`.** The socket handshake reads the session
  through `auth`'s exported `GetSessionUseCase`; the session-id-to-connection map
  (`SessionSocketRegistry`) lives in `system` because `system` owns the only
  gateway. `dependency-cruiser`'s `no-sibling-module-import` gained an `auth`
  exception. A session delete reaches the sockets through `SessionCloseBus`, a
  `@Global` in-process emitter in `src/realtime/` that neither module imports the
  other to use. The Origin check is `OriginCheckedIoAdapter` (an `allowRequest`
  against `SOCKET_ALLOWED_ORIGINS`), not a gateway `cors` option.

- **`NO_SESSION`** was added to `ERROR_CODES` in the guard slice (the 401) and
  reused by the handshake refusal and the mid-session socket close.

- **Two new env vars**, each added to `env.ts`, `.env.example` and `env.spec.ts`
  together: `AUTH_REGISTRATION_OPEN` (`true`|`false`) and
  `SOCKET_ALLOWED_ORIGINS` (comma-separated; the Vite dev origin by default).

- **The dev proxy strips its `/api` prefix.** `vite.config.ts`'s
...
- **The OpenAPI generator boots in tsx preview mode, which emits no
  ...
  handler needs is injected into its use case (all-token `@Inject`) and returned
  as data, never injected on the controller. `registrationOpen` rides on
  `GetSessionUseCase`'s result for this reason.

- **The web reads the session with one hook.** `features/session/useSession()`
  over the generated query hook; `_authed.tsx` resolves it in `beforeLoad`
  through the same cache and redirects with `search.redirect`. The top bar is
  session-aware (empty slot while pending, account menu signed in, standalone
  switcher signed out). The 401 handler is a callback `fetcher.ts` exposes and
  `main.tsx` wires. `orval.config.ts` dropped `query: { useQuery: true }`, which
  had been forcing every operation to a query hook.

- **`(manual)` not run:** the one browser confirmation in the one-origin slice
  has no harness in the repo; everything else there is asserted from config.
