...
- **<http://localhost:3000>** — the API on its own, for `curl`. There is **no
  page here**: no Swagger UI is mounted and `/` is not a route, so a browser
  gets a 404. The OpenAPI document is a committed file,
  [`apps/api/openapi.json`](apps/api/openapi.json), generated by `make
  gen-open-api` and read by Orval. Paste it into any OpenAPI viewer to read it.
  Serving it as a live docs page is **not wired yet**.
...
| `make db-studio` | Open drizzle-kit studio against the local database, at <https://local.drizzle.studio>. A browser table browser. |
...
| `apps/web/src/theme.ts` and `theme.css` | `make generate`, from [`apps/web/design-tokens.json`](apps/web/design-tokens.json) |
...
| `OBSERVE_SERVICE_ID` | `tormented-path-api` | Names this service in the dashboard. |
...
seconds, then open the dashboard and look for the `tormented-path-api` service. Traces
for `GET /server-meta` and `POST /auth/sign-in` should be there. If the `tormented-path-api` service
...
**To look at them:** `make db-studio`, then open <https://local.drizzle.studio>
— drizzle-kit's studio is hosted there and connects back to your local
database, so the URL is not a localhost one.
